...
> **Certification extract:** This repository is a standalone copy of [`gotgenes/pi-packages` `packages/pi-permission-system`](https://github.com/gotgenes/pi-packages/tree/main/packages/pi-permission-system) at `1256ae72a1ed2b06ac6cf3ffe9dad112670ab91a` (v26.3.0). Permission policy logic is unchanged. The only packaging changes are a self-contained `tsconfig.json` and replacing pnpm `catalog:` versions so `pi install git:` can run `npm install`.
...
1. Optional: copy or edit `~/.pi/agent/extensions/pi-permission-system/config.json`.
   With no user file, the package default is used: allow ordinary work, **ask** before leaving the workspace, recursive deletes (`rm -r` / `rm -rf` / `rm -fr`), `sudo`, `.env`, `~/.ssh/*`, or `settings.json`. Ordinary `rm`/`del` of a single file is allowed. Nothing is denied by default.
