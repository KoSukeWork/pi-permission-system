...
- Align `PolicyLoaderOptions.packagedDefaultConfigPath` with its runtime contract by accepting `null` to disable the packaged fallback.
- Make the multi-path permission tests pass strict type checking and add a repeatable focused `test:permission-paths` command.
...
### Changed
- Packaged default bash policy asks only for recursive deletes (`rm -r` / `rm -rf` / `rm -fr`) and `sudo`. Ordinary `rm` / `del` of a single file is allowed. Workspace-external access and sensitive-path asks are unchanged.

## [26.3.1] - 2026-08-19

### Changed
- Packaged default policy is now allow-by-default and asks for workspace-external paths, deletes, sudo, and sensitive files. No default deny rules.
